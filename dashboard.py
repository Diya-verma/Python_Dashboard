...
st.set_page_config(page_title='sale analysis',page_icon=':bar_chart:',layout='wide')
...
f1=st.file_uploader(":file_folder: upload a file",type=["txt","csv","xlsx","xls"])
if f1 is not None:
    filename=f1.name
    st.write(filename)
    df=pd.read_csv(filename,encoding="ISO-8859-1")
else:
    os.chdir(r"C:\Users\User\Desktop\Python_dashboard")
...
with col1:
    date1=pd.to_datetime(st.date_input("Start",startDate))
with col2:
    date2=pd.to_datetime(st.date_input("End",endDate))
...
#Create a TreeMap Based on Region,Category,sub_category
...
data1.update_layout(title="Relationship Between Sales and Profit Using Sactter Plot.",
